...
use crate::{
    graph::edge::Hold, graph::new_db_connection, graph::vertex::Identity, upstream::Platform,
    util::naive_now,
};
...
async fn test_smoke_dotbit_reverse_record() -> Result<(), Error> {
    ...
    let dotbit_record = Identity::find_by_platform_identity(&db, &Platform::Dotbit, "justing.bit")
        .await?
        .expect("Record not found");
